import React, { /* useState, useEffect, useRef */memo, useRef } from 'react';
    ...
    <div className="numbers_container">
      <div className="numbers_wrapper">
        <div className="number_value" style={{ fontSize: `${Math.max((Math.abs(values[0]) / divider) * 100, 70)}%` }}>
          <div ref={isVisibleRef1} className={(values[0] < 0) ? 'negative' : ''}>
            {isVisible1 && (<CountUp easingFn={easingFn} start={0} delay={0.7} end={values[0]} decimals={1} duration={4} separator="," decimal={decimal} useEasing prefix={(values[0] > 0) ? prefix : ''} suffix={suffix} />)}
          </div>
        </div>
      ...
      <div className="numbers_wrapper">
        <div className="number_value" style={{ fontSize: `${(Math.abs(values[1]) / divider) * 100}%` }}>
          <div ref={isVisibleRef2}>
            {isVisible2 && (<CountUp easingFn={easingFn} start={0} delay={0.7} end={values[1]} decimals={1} duration={4} separator="," decimal={decimal} useEasing prefix={(values[1] > 0) ? prefix : ''} suffix={suffix} />)}
          </div>
        </div>
